...
When you get into systems design, eventually you are going to have to deal with caching.  I am currently writing [my serverless MUD](/posts/2026/2026-05-01-cf-workers-3.html) and part of that system requires an automatic registration of users hitting the API.  This means that every single API call reads from [Cloudflare D1] in the hot path.  Reading from D1 is incredibly fast, but it's not latency-free, particularly during periods of high load.
...
![Cloudflare Caching Options](/assets/images/2026/Jul01-banner.png)
...
export class KVCacheManager<T> implements CacheManager<T> {
  ...
  async get(key: string): Promise<T | undefined> {
    const value = await this.kv.get<T>(key, { type: "json", cacheTtl: 60 });
